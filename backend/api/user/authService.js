...
const login = (req, res, next) => {
    ...
    User.findOne({email}, (err, user) => {
        ...
        } else if (user && bccrypt.compareSync(password, user.password)) {
...
const signup = (req, res, next) => {
    ...
    if(!password.match(passwordRegex)) {
        return res.status(400).send({errors: [
            "Senha precisa ter: uma letra maiuscula, uma letra minuscula, um numero, um caractere especial(@$%^) e tamnaho entre 6-20."
        ]})
    }
